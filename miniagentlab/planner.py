...
class LLMPlanner(Planner):
    ...
    def _build_prompt(self, task: str, tools: ToolRegistry, context: PlannerContext | None = None) -> str:
        ...
        return (
            ...
            "Use memory references exactly as \"$memory.step_id\" when a later step needs an earlier step output. "
            "If a step output is an object and a later step needs one field, use \"$memory.step_id.field\". "
            "Do not use \"$step_id.result\", \"$steps.step_id\", or any other reference syntax.\n"
            ...
            "If step_2 needs step_1 output, use an arg value like \"$memory.step_1\". "
            "If step_4 needs the store_id returned by step_3, use \"$memory.step_3.store_id\".\n"
